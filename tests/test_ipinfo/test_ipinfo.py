...
def test_ipinfo_download_failure(tmp_path: Path) -> None:
    ...
    with (
        patch("httpx.Client.get", side_effect=Exception("Download failed")),
...
def test_db_initialization_retry(tmp_path: Path) -> None:
    db = IPInfoManager(token="test", db_path=tmp_path / "test.mmdb")
    with (
        patch("httpx.Client.get", side_effect=Exception("First fail")),
...
def test_database_retry_success(tmp_path: Path) -> None:
    ...
    with (
        patch("httpx.Client.get", side_effect=side_effect_function),
...
def test_db_age_check(tmp_path: Path) -> None:
    ...
    db_path.touch()  # Create the file so exists() returns True
...
def test_invalid_token_handling(tmp_path: Path) -> None:
