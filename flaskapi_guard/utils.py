...
def extract_client_ip(
    ...
    1. If no trusted proxies are defined, the connecting IP (request.remote_addr) is
       always used
...
def check_ip_country(
    ...
    Args:
        request (str | Request):
            The Flask request object or IP string.
        config (SecurityConfig):
            The security configuration object.
        geo_ip_handler (GeoIPHandler):
...
def _check_cloud_providers(ip: str, config: SecurityConfig) -> bool:
...
def detect_penetration_attempt(request: Request) -> tuple[bool, str]:
    ...
    Args:
        request (Request):
            The Flask request object to analyze.
