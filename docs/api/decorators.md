...
title: Security Decorators API - FlaskAPI Guard
description: API reference for managing security decorators in FlaskAPI Guard
keywords: security decorators, flaskapi guard, security extension, python api reference
---

Security Decorators
======================

The decorators module provides route-level security controls that can be applied to individual Flask endpoints. These decorators offer fine-grained control over security policies on a per-route basis, complementing the global extension security features.
...
The decorators work in conjunction with the FlaskAPIGuard extension to provide comprehensive protection:

1. **Route Configuration**: Decorators configure route-specific settings
2. **Extension Processing**: FlaskAPIGuard reads decorator configurations and applies them
3. **Override Behavior**: Route-specific settings can override global extension settings
...
# Set decorator handler (required for integration)
...
Decorators integrate with the extension's error handling system. When decorator conditions are not met, appropriate HTTP responses are returned:
