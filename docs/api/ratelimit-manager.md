...
title: RateLimitManager - FlaskAPI Guard
description: API reference for the RateLimitManager class in FlaskAPI Guard, handling rate limiting functionality
keywords: rate limiting, api security, flask, rate limit handler
---

RateLimitManager
================

The `RateLimitManager` is responsible for managing rate limiting functionality in FlaskAPI Guard. It supports both in-memory rate limiting using timestamp tracking and distributed rate limiting using Redis.
...
# Add extension with rate limiting
...
- [FlaskAPIGuard](./security-middleware.md)
