...
title: Security Headers Manager - FlaskAPI Guard
...
```python
headers = security_headers_manager.get_headers(
...
```python
is_valid = security_headers_manager.validate_csp_report(
    report={
        "csp-report": {
...
```python
security_headers_manager.initialize_redis(redis_handler)
```
...
```python
security_headers_manager.initialize_agent(agent_handler)
```
...
- [FlaskAPIGuard](security-middleware.md) - Extension integration
