...
title: Advanced Security Decorators - FlaskAPI Guard
...
Globally, FlaskAPI Guard may be configured to detect suspicious patterns in requests. This decorator allows you to override that setting for specific endpoints where the global rules may not be appropriate.
...
```python
@app.route("/api/code-submission", methods=["POST"])
...
Advanced decorators integrate with the extension's error handling:
