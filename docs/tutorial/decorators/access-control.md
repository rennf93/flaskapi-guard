...
title: Access Control Decorators - FlaskAPI Guard
...
```python
@app.route("/api/compliance-sensitive")
...
```python
@app.route("/api/residential-only")
...
```python
# The extension will use custom error messages if configured
