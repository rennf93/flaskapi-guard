...
title: Rate Limiting Decorators - FlaskAPI Guard
...
@app.route("/api/admin/data-purge", methods=["DELETE"])
...
@app.route("/api/search/complex", methods=["POST"])
...
@app.route("/api/after-hours/order", methods=["POST"])
...
@app.route("/api/medium/data")
@guard_deco.rate_limit(requests=100, window=3600)
def medium_data():
    return {"data": "medium complexity"}

# Complex operations: Low limits
@app.route("/api/complex/analysis", methods=["POST"])
