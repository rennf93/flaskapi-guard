...
class SecurityConfig(BaseSettings):
    ...
    All settings can be configured via environment variables with FLASKAPI_GUARD_ prefix.
    """
```
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enable_agent` | bool | False | Enable FlaskAPI Guard Agent integration |
...
All settings can be configured using environment variables with the `FLASKAPI_GUARD_` prefix:

```bash
export FLASKAPI_GUARD_ENABLED=true
export FLASKAPI_GUARD_DETECTION_SEMANTIC_THRESHOLD=0.8
export FLASKAPI_GUARD_REDIS_HOST=redis.example.com
export FLASKAPI_GUARD_AGENT_API_KEY=your-api-key
```
...
- [Security Extension](../../api/security-extension.md) - Using SecurityConfig with the extension
