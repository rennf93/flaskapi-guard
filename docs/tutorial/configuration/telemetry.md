---
title: Telemetry
description: OpenTelemetry, Logfire, and event/metric/log muting for FlaskAPI Guard
---

# Telemetry

FlaskAPI Guard emits security events and request metrics through a composable telemetry pipeline provided by [guard-core](https://github.com/rennf93/guard-core). Events can be muted, metrics can be muted, individual security-check logs can be muted, and exports to OpenTelemetry and Logfire are opt-in.
...
=== "uv"
    ...
    ```bash
    uv add flaskapi-guard "guard-core[otel]"
    ```

=== "poetry"

    ```bash
    poetry add flaskapi-guard "guard-core[otel]"
    ```

=== "pip"

    ```bash
    pip install flaskapi-guard "guard-core[otel]"
    ```
...
=== "uv"
    ...
    ```bash
    uv add flaskapi-guard "guard-core[logfire]"
    ```

=== "poetry"

    ```bash
    poetry add flaskapi-guard "guard-core[logfire]"
    ```

=== "pip"

    ```bash
    pip install flaskapi-guard "guard-core[logfire]"
    ```
...
4. Confirm the extension is installed via `FlaskAPIGuard(app, config=config)` on the same `app` instance used to serve requests.
