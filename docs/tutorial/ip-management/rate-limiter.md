...
title: Rate Limiting - FlaskAPI Guard
description: Learn how to implement rate limiting in your Flask application using FlaskAPI Guard
keywords: rate limiting, api security, ddos protection, request throttling, flask
---

Rate Limiting
=============

Rate limiting is a crucial security feature that protects your API from abuse, DoS attacks, and excessive usage. FlaskAPI Guard provides a robust rate limiting system through the dedicated `RateLimitManager` class.
...
FlaskAPI Guard implements a sliding window rate limiting algorithm:
...
FlaskAPI Guard supports two rate limiting storage backends:
