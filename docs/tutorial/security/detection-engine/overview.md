...
The FlaskAPI Guard Detection Engine is an enhanced pattern-based threat detection system that provides protection against common web application attacks through timeout-protected pattern matching and optional heuristic analysis.

## Overview

The Detection Engine is an evolution of FlaskAPI Guard's suspicious patterns system, introducing:
...
- Wrapping pattern execution in thread-based timeouts
...
```mermaid
flowchart TB
    Request[Flask Request] --> DPA[detect_penetration_attempt]
...
sus_patterns_handler.clear_custom_patterns()
```
...
The timeout mechanism uses thread-based execution to prevent patterns from running indefinitely:
