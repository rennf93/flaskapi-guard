...
This document provides detailed information about each component of the FlaskAPI Guard Detection Engine, including their actual implementation, capabilities, and usage.
...
Located in `flaskapi_guard/detection_engine/preprocessor.py`
...
Located in `flaskapi_guard/detection_engine/compiler.py`
...
- Uses thread-based execution with configurable timeout
- Runs pattern matching in thread pool to prevent blocking
- Returns timeout indicator instead of hanging

## SemanticAnalyzer

Located in `flaskapi_guard/detection_engine/semantic.py`
...
Located in `flaskapi_guard/detection_engine/monitor.py`
...
```python
def detect(self, content: str, **kwargs) -> dict:
...
- Pattern matching runs in thread pool for timeout protection
